...
sys.path.append(str(pathlib.Path('/export/home/pblab/data/MatlabCode/PBLabToolkit/CalciumDataAnalysis/python-ca-analysis-bloodflow')))
...
class SingleFovViz:
    ...
    def __attrs_post_init__(self):
        self.analog_vectors = [np.nan_to_num(vec) for vec in [self.fov.analog_analyzed.stim_vec,
                                                              self.fov.analog_analyzed.juxta_vec,
                                                              self.fov.analog_analyzed.run_vec]]
        if self.fov.analog_analyzed.occluder:
            self.analog_vectors.append(np.nan_to_num(self.fov.analog_analyzed.occluder_vec))
    ...
    def draw(self):
        ...
        if self.fov.with_analog:
            ...
            self._draw_analog_plots(gs, colors)
        if self.save:
            self.fig.savefig(str(self.fov.metadata.fname)[:-4] + '_summary.pdf', transparent=True, dpi=300, format='pdf')
    ...
    def _create_rect_patches(self, fps: float, height: int):
        ...
        colors: List[Tuple[float, float, float]] = [colors[7], colors[11], colors[15], colors[19]]  # manually picked due to fitting colors
        vec_and_colors = [(vec, colors[idx]) for idx, vec in enumerate(self.analog_vectors)]
    ...
    def _scat_spikes(self, ax):
        ...
        spikes = dff_tools.locate_spikes_peakutils(self.fov.fluo_trace, self.fov.metadata.fps)
    ...
    def _draw_analog_plots(self, gs: gridspec.GridSpec, colors):
        ...
        for idx, (label, data, color) in enumerate(zip(labels, self.analog_vectors, colors), self.axes_for_dff):
...
if __name__ == '__main__':
    ...
    # home = pathlib.Path('/')
    folder = home / pathlib.Path(r'data/David/NEW_crystal_skull_TAC_161018/DAY_0_ALL/619_HYPO_DAY_0/')
    fov = 1
