...
import h5py
from calium_bflow_analysis.trace_converter import ConversionMethod, RawTraceConverter
from calcium_bflow_analysis.analysis_gui import AnalysisGui
...
def main(
    filename=None, save_file=False, run_gui=True, do_calcium=True, do_vessels=True
) -> Dict:
    ...
    if filename is None:
        ...
        filename = filedialog.askopenfilename(
            title="Choose a stack for cell ROIs",
            filetypes=[
                ("Tiff Stack", "*.tif"),
                ("HDF5 Stack", "*.h5"),
                ("HDF5 Stack", "*.hdf5"),
    ...
    fpath = str(
        ...
        / Path(f"vessel_neurons_analysis_{Path(filename).name[:-4]}")
    )
    ...
    if ca_analysis:
        img_neuron, time_vec, fluo_trace, rois = determine_manual_or_auto(
    ...
    if bloodflow_analysis:
        ...
        if True:
            struct_name = "mv_mpP"
            vessel_lines, diameter_data, img_vessels = import_andy_and_plot(
                filename=patrick_mat, struct_name=struct_name, colors=colors
            )
            ...
            if ca_analysis:
                idx_of_closest_vessel = find_closest_vessel(
                    rois=rois, vessels=vessel_lines
                )

                plot_neuron_with_vessel(
                    rois=rois,
                    vessels=vessel_lines,
                    closest=idx_of_closest_vessel,
                    img_vessels=img_vessels,
    ...
    if gui.analog_trace.get():
        ...
        analog_data = pd.read_table(
            analog_data_fname, header=None, names=["stimulus", "run"], index_col=False
        )
        an_trace = AnalogTraceAnalyzer(filename, analog_data)
        an_trace.run()
        sliced_fluo: xr.DataArray = an_trace * return_vals[
            "fluo_trace"
        ]  # Overloaded __mul__
...
def determine_manual_or_auto(
    ...
    try:
        ...
        img_neuron, time_vec, fluo_trace, rois = parse_npz_from_caiman(
            filename=corresponding_npz, fps=fps
        )
    except StopIteration:
        img_neuron, time_vec, fluo_trace, rois = draw_rois_and_find_fluo(
...
def parse_npz_from_caiman(filename: Path, fps=15.24):
    ...
    fluo_trace = caiman_funcs_for_comparison.detrend_df_f_auto(
        full_dict["A"], full_dict["b"], full_dict["C"], full_dict["f"], full_dict["YrA"]
    )  # offline pipeline
    ...
    converted_trace = RawTraceConverter(
        conversion_method=ConversionMethod.NONE, raw_data=fluo_trace
    ).convert()
...
def draw_rois_and_find_fluo(
    ...
    final_fluo = RawTraceConverter(
        conversion_method=con_method, raw_data=fluorescent_trace
    ).convert()
    ...
    try:
        ...
        with h5py.File(filename, driver="core") as f:
            ...
            for idx in range(num_of_vessels):
                ...
                line_x1, line_x2 = f[andy["Vessel"][idx, 0]]["vessel_line/position/xy"][
                    0, :
                ]
                line_y1, line_y2 = f[andy["Vessel"][idx, 0]]["vessel_line/position/xy"][
                    1, :
                ]
                vessel_lines.append(
                    Line(x1=line_x1, x2=line_x2, y1=line_y1, y2=line_y2)
                )

            for idx in range(num_of_vessels):
                ax_dia[idx].plot(
                    np.arange(diameter_data[idx].shape[0]),
                    diameter_data[idx],
                    color=colors[idx],
                )
                ax_ves.plot(
                    [vessel_lines[idx].x1, vessel_lines[idx].x2],
        ...
        for idx in range(num_of_vessels):
            ...
            line_x1, line_x2 = andy[struct_name][0, idx]["Vessel"]["vessel_line"][0][0][
                0
            ][0][0][0][0][0][:, 0]
            line_y1, line_y2 = andy[struct_name][0, idx]["Vessel"]["vessel_line"][0][0][
                0
            ][0][0][0][0][0][:, 1]
        ...
        for idx in range(num_of_vessels):
            ax_dia[idx].plot(
                np.arange(diameter_data[idx].shape[1]),
                diameter_data[idx].T,
                color=colors[idx],
            )
            ax_ves.plot(
...
def find_closest_vessel(rois: List[np.ndarray], vessels: List) -> np.array:
    ...
    for idx, vessel in enumerate(vessels):
        com_vessels[idx, :] = (
            np.mean((vessel.x1, vessel.x2)),
            np.mean((vessel.y1, vessel.y2)),
        )

    idx_of_closest_vessel = np.zeros((len(rois)), dtype=int)
    for idx, roi in enumerate(rois):
        com_x, com_y = roi[0], roi[1]  # center of ROI
        helper_array = np.tile(
            np.array([com_x, com_y]).reshape((1, 2)), (com_vessels.shape[0], 1)
        )
...
def plot_neuron_with_vessel(
    ...
    for idx, vessel in enumerate(vessels):
        try:
            ...
            if len(cur_cells) > 0:
                ax_img.plot(
                    [vessel.x1, vessel.x2], [vessel.y1, vessel.y2], color=colors[idx]
                )
                plotted_vessels_idx.append(idx)
                drawn_cells_idx.append(cur_cells[0])
                # Draw the first cell
                circ = plt.Circle(
                    (rois[cur_cells[0]][0], rois[cur_cells[0]][1]),
    ...
    for idx in drawn_cells_idx:
        ...
        ax_neurons[-1].plot(
            time_vec[idx, :],
            fluo_trace[idx, :],
            color=colors[plotted_vessels_idx[counter]],
        )
        ...
        if cur_closest_vessel in plotted_vessels_idx:
            ...
            counter += 1
            corr = (
                np.correlate(closest_vessel, fluo_trace[idx, :], mode="same")
                / fluo_trace[idx, :].shape[0]
            )
...
def display_data(fname):
    ...
    for idx, vessel in enumerate(data["vessel_lines"]):
        ax_img.plot([vessel.x1, vessel.x2], [vessel.y1, vessel.y2], color=colors[idx])
        try:
        ...
        for loc in cur_cells:
            circ = plt.Circle(
                (data["rois"][loc][0], data["rois"][loc][1]),
                edgecolor=colors[idx],
                fill=False,
            )
            ax_img.add_artist(circ)
...
def run():
    ...
    if pre_gui.calcium_over_days.get():
        ...
        foldername = filedialog.askdirectory(
            title="Choose the base directory containing all files",
            initialdir=r"/data/David",
        )
        result = AnalyzeCalciumOverTime(foldername, pre_gui.analog.get())
    else:
        filename = filedialog.askopenfilename(
            title="Choose a stack for analysis",
            filetypes=[
                ("Tiff Stack", "*.tif"),
                ("HDF5 Stack", "*.h5"),
                ("HDF5 Stack", "*.hdf5"),
        ...
        result = Analyze(
            filename,
            pre_gui.calcium_after_caiman.get(),
...
if __name__ == "__main__":
    ...
    result = AnalyzeCalciumOverTime(
        Path(r"/data/David/THY_1_GCaMP_BEFOREAFTER_TAC_290517")
    ).run_batch_of_timepoint()
